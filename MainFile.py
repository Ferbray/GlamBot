...
class Branch(ClsBranch):
	...
	async def join_registration_branch(self, ans: Message):
		...
		"'%s', '%s',array[0],array[0],array[0],array[0],array[0],"
		"array[0],array[0],array[0],array[0],array[0],0,0,0,0,0,0)")
...
async def find_server(ans: Message):
	...
	for sess in sessions_list:
		...
		if session_player[0][f"{sess[1]}"] < 100 or
		(session_player[0]["status_player"] >= ... and session_player[0][f"{sess[1]}"] < 110):
...
class Branch(ClsBranch):
	...
	async def exit_market_branch(self, ans: Message):
        ...
        await ans("Окей, выходим!\nНажмите на кнопку <<Домой>>, чтобы вернуться", keyboard=back_market_keyboard, attachment=)
